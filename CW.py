...
def line_search(arr, target):
    for i in range(len(arr)):
        if arr[i] == target:
            return f'{i} - позиция искомого {target}'
    return -1
...
# Логарифмическая сложность означает, что время выполнения алгоритма увеличивается логарифмически
# с увеличением размера входных данных. Логарифм — это количество шагов, которые требуется проделать для получения результата
